...
df_amr = pd.read_csv("../data/amr_metadata_full.csv", dtype=str, engine='python')
output_file = "../data/amr_metadata_extended.csv"
...
is_meta = df_amr["organism"].str.contains("metagenome", case=False, na=False)
df_amr["organism_type"] = is_meta.map({True: "Metagenome", False: "Isolate"})
...
df_amr["metagenome_category"] = df_amr.apply(decide_category, axis=1)

# Output file
df_amr.to_csv(output_file, index=False)