...
out_dir    = "../data/who_category_timelines_wastewater_europe"
...
df = df[
    (df["metagenome_category"] == "wastewater") &
    (df["geo_loc_name_country_continent_calc"] == "Europe")
]

# if nothing left, exit quietly
if df.empty:
    print("No wastewater-Europe samples found — nothing to plot.")
    quit()
...
for who_cat, drugs in drug_classes.items():
    ...
    fname = os.path.join(out_dir,
                         f"timeline_{who_cat.replace(' ', '_').lower()}_wastewater_europe.png")
