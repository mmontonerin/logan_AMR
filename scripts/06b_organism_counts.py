...
def process_csv_files(input_file, output_file, full_counts_file, chunk_size=1000000):
    """
    Process large CSV files in chunks to generate organism counts, and save results to separate files.
    ...
    with pd.read_csv(input_file, chunksize=chunk_size, low_memory=False) as reader:
        for i, chunk in enumerate(reader):
            ...
            # Count occurrences of each "organism" in the chunk
            for organism in chunk['organism'].dropna():
                organism_counts[organism] = organism_counts.get(organism, 0) + 1

            # Print progress
            sys.stdout.flush()
            print(f"Processed chunk {i + 1} with {len(chunk)} rows, current total count: {sum(organism_counts.values())}")
...
if __name__ == "__main__":
    ...
    output_file = "./data/organism_counts_top15.csv"
    full_counts_file = "./data/organism_counts_full.csv"
