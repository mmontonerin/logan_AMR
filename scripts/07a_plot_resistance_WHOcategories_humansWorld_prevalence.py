...
out_dir = "../data/who_prevalence_human_world"
...
amr_df = amr_df[
    (amr_df["metagenome_category"] == "human")
]
sra_df = sra_df[
    (sra_df["metagenome_category"] == "human")
]

if amr_df.empty or sra_df.empty:
    print("No human rows in one of the tables - nothing to plot.")
    quit()
...
timeline = pd.period_range("2016Q1", "2021Q4", freq="Q")
...
for who_cat, drugs in drug_classes.items():
    ...
    fname = os.path.join(out_dir,
                         f"prev_{who_cat.replace(' ', '_').lower()}_human_world.png")
